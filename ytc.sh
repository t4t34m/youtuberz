...
found=$(notify-send -u normal -t 6500 "Username" "$YTD_UserName" -i /opt/backbox/youTube_Checker/icon/youtube.ico)
YTD_OwnerUrls=$(wget -qO- "https://www.youtube.com/channel/$YTD_channelLink" | grep -iPo '(?<=ownerUrls":..)(.*)(?="],"avatar)')
found=$(notify-send -u normal -t 6500 "Owner Url" "$YTD_OwnerUrls" -i /opt/backbox/youTube_Checker/icon/h.png)
YTD_API_KEY=$(wget -qO- "https://www.youtube.com/channel/$YTD_channelLink" | grep -iPo '(?<=INNERTUBE_API_KEY...)(.*)(?=...INNERTUBE_API_VERSION)')
found=$(notify-send -u normal -t 6500 "Your APIKEY IN CASE " "$YTD_API_KEY" -i /opt/backbox/youTube_Checker/icon/apikey.png)
